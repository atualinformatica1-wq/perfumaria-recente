...
export class App extends DurableObject {
  ...
  constructor(ctx: DurableObjectState, env: Record<string, unknown>) {
    ...
    // Inicializa as tabelas do Banco de Dados SQLite em memória/storage do Cloudflare
    this.initDb();
  ...
  private initDb() {
    ...
    // Verifica se as configurações iniciais existem
  ...
  private setupRoutes() {
    ...
    this.app.post("/api/settings", async (c) => {
      ...
      return c.json({ ok: true, message: "Configurações salvas com sucesso!" });
    });
    ...
    this.app.delete("/api/products/:id", (c) => {
      ...
      return c.json({ ok: true, message: "Produto removido com sucesso!" });
    });

    // GET Categories
    this.app.get("/api/categories", (c) => {
      const rows = this.ctx.storage.sql
        .exec(`SELECT * FROM categories ORDER BY name ASC`)
        .toArray();
      return c.json(rows);
    });
    ...
    // POST Consultation / Lead (Tracks when a user clicks WhatsApp)
    this.app.post("/api/leads", async (c) => {
      ...
      this.ctx.storage.sql.exec(
        ...
        body.product_id || "",
        body.product_name || "",
        body.customer_name || "Visitante da Loja",
        body.customer_phone || "",
        body.notes || "",
        Date.now()
      );
    ...
    this.app.get("/api/leads", (c) => {
      const rows = this.ctx.storage.sql
        .exec(`SELECT * FROM leads ORDER BY created_at DESC LIMIT 50`)
        .toArray();
      return c.json(rows);
    });
    ...
    this.app.get("/api/stats", (c) => {
      const totalProducts = this.ctx.storage.sql
        .exec(`SELECT COUNT(*) as c FROM products WHERE is_active = 1`)
        .one().c as number;

      const totalCategories = this.ctx.storage.sql
        .exec(`SELECT COUNT(*) as c FROM categories`)
        .one().c as number;

      const totalLeads = this.ctx.storage.sql
        .exec(`SELECT COUNT(*) as c FROM leads`)
        .one().c as number;

      const featuredCount = this.ctx.storage.sql
        .exec(`SELECT COUNT(*) as c FROM products WHERE is_featured = 1 AND is_active = 1`)
        .one().c as number;
